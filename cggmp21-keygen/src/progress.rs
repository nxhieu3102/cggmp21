...
mod requires_std {
    ...
    impl PerfProfiler {
        ...
        pub fn get_report(&self) -> Result<PerfReport, ProfileError> {
            if let Some(err) = self.error.clone() {
                ...
                report.peak_rss_kb = self.peak_rss_kb;
                Ok(report)
            }
        }
        ...
        fn try_trace_event(&mut self, event: Event) -> Result<(), ProfileError> {
            ...
            // Update peak RSS memory usage
            self.system.refresh_process(Pid::from(std::process::id() as usize));
    ...
    impl fmt::Display for PerfReport {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            ...
            writeln!(f, "  - Peak RSS: {:.2} MiB", self.peak_rss_kb as f64 / 1024.0)?;
            if self.display_io {
                writeln!(
                    f,
