use alloc::vec::Vec;
/// Hierarchical threshold key generation protocol
///
/// This protocol is used to generate a shared secret key among a group of parties.
/// The key is generated by a hierarchical threshold scheme, where the threshold is
...
///
/// Rank of each shareholder is in range 0..=t-1
...
{
    // Note 1: share of each party is generated by the polynomial derivative
    //         use .nth_derivative_at(x, rank) instead of .value(x)
    // Note 2: the ranks is exchanged when set up networking
