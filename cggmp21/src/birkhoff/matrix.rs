...
impl Matrix {
    ...
    pub fn multiply(&self, other: &Self) -> Result<Self, Error> {
        ...
        if self.cols != other.rows {
            return Err(Error::MatrixError("Inconsistent dimensions for multiplication".to_string()));
        }
    ...
    fn get_gauss_elimination(&self) -> Result<(Self, Self, usize), Error> {
        ...
        for i in 0..self.rows {
            ...
            for j in i+1..self.rows {
                ...
                let temp_result_identity_slice = Self::multi_scalar(&row_lower_i, &inverse_diagonal_component);
