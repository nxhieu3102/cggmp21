...
use crate::signing::{
    msg::{MsgReliabilityCheck, MsgRound1a, MsgRound1b, MsgRound2, MsgRound3, MsgRound4},
...
pub struct SigningState<E: Curve, L: SecurityLevel, D: Digest + Clone + 'static> {
    ...
    /// Random nonce rho_i
    pub rho_i: Option<Integer>,
    /// Random nonce nu_i
    pub nu_i: Option<Integer>,
    /// Encrypted values
    ...
    NonZero<Point<E>>: AlwaysHasAffineX<E>,
{
    ...
    pub fn new(
        ...
        let (mut x_i, mut X) = if let Some(VssSetup { I, ranks, .. }) = &key_share.core.vss_setup {
            if let Some(ref ranks) = ranks {
                ...
                let lambda =
                    (0..signing_parties.len()).map(|j| lagrange_coefficient_at_zero(j, &I));
                let X = lambda
                    .zip(&X)
    ...
    pub fn round1a_generate_message(&mut self) -> Result<MsgRound1a<E>, SigningError> {
        ...
        let N_i = &R_i.N;
        let rho_i = Integer::gen_invertible(N_i, &mut self.rng);
        let nu_i = Integer::gen_invertible(N_i, &mut self.rng);
        ...
        let K_i = ek_i
            ...
            .map_err(|e| {
                SigningError(SigningErrorReason::Bug(format!(
                    "Failed to encrypt K_i: {:?}",
                    e
                )))
            })?;
        let G_i = ek_i
    ...
    pub fn create_reliability_check(&mut self) -> Result<MsgReliabilityCheck<D>, SigningError> {
        let round1a_msgs = self.state.round1a_msgs.as_ref().ok_or_else(|| {
            SigningError(SigningErrorReason::Bug(
                "Round 1a messages not set".to_string(),
            ))
        })?;

        let my_round1a = self.state.my_round1a.as_ref().ok_or_else(|| {
            SigningError(SigningErrorReason::Bug(
                "My round 1a message not generated".to_string(),
            ))
        })?;
        ...
        let h_i =
            udigest::hash_iter::<D>(round1a_msgs.iter_including_me(my_round1a).map(|msg| Echo {
                sid: self.state.sid,
                msg,
            }));
    ...
    pub fn validate_round1b_proofs(&mut self) -> Result<(), SigningError> {
            ...
            round1a_msgs.iter_indexed().zip(round1b_msgs.iter_indexed())
        {
            ...
            .map_err(|e| {
                SigningError(SigningErrorReason::Aborted(format!(
                    "Invalid enc-elg proof from party {}: {:?}",
                    j, e
                )))
            })?;
        }
    ...
    pub fn round2_generate_messages(&mut self) -> Result<Vec<(u16, MsgRound2<E>)>, SigningError> {
        ...
        for (j, _, round1a_msg) in round1a_msgs.iter_indexed() {
            ...
            let r_ij = random_in_range(&mut self.rng, &Integer::from(0), N_i);
            let hat_r_ij = random_in_range(&mut self.rng, &Integer::from(0), N_i);
            let s_ij = random_in_range(&mut self.rng, &Integer::from(0), N_i);
            let hat_s_ij = random_in_range(&mut self.rng, &Integer::from(0), N_i);

            let beta_ij = Integer::from_rng_pm(&J, &mut self.rng);
            let hat_beta_ij = Integer::from_rng_pm(&J, &mut self.rng);
            ...
            let D_ji = {
                let gamma_i_times_K_j = enc_j
                    ...
                    .map_err(|e| {
                        SigningError(SigningErrorReason::Bug(format!(
                            "Failed to compute gamma_i * K_j: {:?}",
                            e
                        )))
                    })?;
                let neg_beta_ij_enc = enc_j.encrypt_with(&(-&beta_ij), &s_ij).map_err(|e| {
                    SigningError(SigningErrorReason::Bug(format!(
                        "Failed to encrypt -beta_ij: {:?}",
                        e
                    )))
                })?;
                enc_j
            ...
            let hat_D_ji = {
                let x_i_times_K_j = enc_j
                    .omul(&Integer::from_scalar(&self.x_i), &round1a_msg.K_i)
                    .map_err(|e| {
                        SigningError(SigningErrorReason::Bug(format!(
                            "Failed to compute x_i * K_j: {:?}",
                            e
                        )))
                    })?;
                let neg_hat_beta_ij_enc =
                    enc_j
                        .encrypt_with(&(-&hat_beta_ij), &hat_s_ij)
                        .map_err(|e| {
                            SigningError(SigningErrorReason::Bug(format!(
                                "Failed to encrypt -hat_beta_ij: {:?}",
                                e
                            )))
                        })?;
                enc_j
            ...
            let psi_aff_ji = pi_aff_batch::non_interactive::prove::<E, D>(
                ...
                pi_aff_batch::PrivateData {
                    batch: vec![
                        ...
                        pi_aff_batch::PrivateElement {
                            x: &Integer::from_scalar(&self.x_i),
                            y: &(-&hat_beta_ij),
                            nonce: &hat_s_ij,
                            nonce_y: &hat_r_ij,
                        },
    ...
    pub fn round3_generate_messages(&mut self) -> Result<Vec<(u16, MsgRound3<E>)>, SigningError> {
        ...
        let alpha_sum = round2_msgs.iter().map(|msg| &msg.D_ji).try_fold(
            Scalar::<E>::zero(),
            |sum, D_ij| {
                let alpha_ij = dec_i.decrypt(D_ij).map_err(|e| {
                    SigningError(SigningErrorReason::Bug(format!(
                        "Failed to decrypt alpha_ij: {:?}",
                        e
                    )))
                })?;
        ...
        let hat_alpha_sum = round2_msgs.iter().map(|msg| &msg.hat_D_ji).try_fold(
            Scalar::zero(),
            |sum, hat_D_ij| {
                let hat_alpha_ij = dec_i.decrypt(hat_D_ij).map_err(|e| {
                    SigningError(SigningErrorReason::Bug(format!(
                        "Failed to decrypt hat_alpha_ij: {:?}",
                        e
                    )))
                })?;
    ...
    pub fn generate_presignature(&mut self) -> Result<Presignature<E>, SigningError> {
            ...
            round1a_msgs.iter_indexed().zip(round3_msgs.iter_indexed())
        {
            ...
            .map_err(|e| {
                SigningError(SigningErrorReason::Aborted(format!(
                    "Invalid hat_psi proof from party {}: {:?}",
                    j, e
                )))
            })?;
        }
        ...
        if Point::generator() * delta != Delta {
            return Err(SigningError(SigningErrorReason::Aborted(
                "Mismatched delta".to_string(),
            )));
        }
        ...
        let delta_inv = delta
            .invert()
            .ok_or_else(|| SigningError(SigningErrorReason::Bug("Zero delta".to_string())))?;
        ...
        let presignature = Presignature {
            Gamma: NonZero::from_point(Gamma)
                .ok_or_else(|| SigningError(SigningErrorReason::Bug("Zero Gamma".to_string())))?,
            hat_k_i,
            hat_chi_i,
            hat_Delta_j,
            hat_S_j,
    ...
    pub fn round4_generate_message(&mut self) -> Result<Option<MsgRound4<E>>, SigningError> {
        let presignature = self.state.presignature.as_ref().ok_or_else(|| {
            SigningError(SigningErrorReason::Bug(
                "Presignature not generated".to_string(),
            ))
        })?;
        ...
        let partial_signature = presignature
            ...
            .map_err(|e| {
                SigningError(SigningErrorReason::Bug(format!(
                    "Failed to issue partial signature: {:?}",
                    e
                )))
            })?;
    ...
    pub fn generate_signature(
        &mut self,
        my_partial_sig: MsgRound4<E>,
    ) -> Result<Signature<E>, SigningError> {
        let round4_msgs = self.state.round4_msgs.as_ref().ok_or_else(|| {
            SigningError(SigningErrorReason::Bug(
                "Round 4 messages not set".to_string(),
            ))
        })?;
        ...
        let message_to_sign = self.state.message_to_sign.ok_or_else(|| {
            SigningError(SigningErrorReason::Bug("No message to sign".to_string()))
        })?;
        ...
        for (j, _, msg_j) in round4_msgs.iter_indexed() {
            ...
            if Gamma * sigma_j != hat_Delta_j * m + hat_S_j * r {
                return Err(SigningError(SigningErrorReason::Aborted(format!(
                    "Invalid partial signature from party {}",
                    j
                ))));
            }
        }
        ...
        sig.verify(&self.shared_public_key, &message_to_sign)
            .map_err(|_| {
                SigningError(SigningErrorReason::Aborted(
                    "Invalid final signature".to_string(),
                ))
            })?;
    ...
    pub fn set_cached_precompute_tables(
        &mut self,
        tables: Vec<precomputed_table::PrecomputeTable>,
    ) {
    ...
    pub fn generate_precompute_table_for_party(
        &self,
        party_index: u16,
    ) -> Result<precomputed_table::PrecomputeTable, SigningError> {
        let party_pos = self
            .state
            .signing_parties
            .iter()
            ...
            .ok_or_else(|| {
                SigningError(SigningErrorReason::InvalidArgs(format!(
                    "Party {} is not in signing parties list",
                    party_index
                )))
            })?;
