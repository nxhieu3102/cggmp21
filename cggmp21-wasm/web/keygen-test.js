...
async function testKeygen() {
  ...
  for (let i = 1; i <= numParties; i++) {
    try {
      console.log(partiesInput[i - 1]);
      const party = new wasmModule.StatefulKeygenProtocol(partiesInput[i - 1]);
      parties.push(party);
